...
$(function(){
  ...
  $('#detail').on('change','select', function (e) {
    ...
    if (kode_bb.val().length > 15) {
      var id = kode_bb.val();
      url = "{{ url('get_harga_bb_default') }}";
      $.ajax({
        url : url,
        type : "POST",
...
function clearSelect2Detail() {
  ...
  $('[name="id_bb[]"]:last,[name="id_warna[]"]:last,[name="id_brand[]"]:last,[name="id_satuan[]"]:last').select2("val", "");
}

function addRow() {
  var detailLine = $('#detail .detailLine').eq(0).html();
  $('#detail .detailLine').last().after('<div class="detailLine form-group">'+detailLine+'</div>');
  $('[name="id_bb[]"]:last,[name="id_warna[]"]:last,[name="id_brand[]"]:last,[name="id_satuan[]"]:last').next(".select2-container").hide();
  select2Run();
  clearSelect2Detail();
  setkg($('[name="id_satuan[]"]'));
  runRibuan();
