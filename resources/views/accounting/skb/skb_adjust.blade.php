                ...
                <div class="widget-body">
                  <form id="formAdd" class="form-horizontal" method="post" data-toggle="validator" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                      <label for="id" class="col-md-2 control-label">No SKN</label>
                      <div class="col-md-2">
                        <select name="skb_id" style="width:100%;" value="" required></select>
                      </div>
                      <!-- <label for="id" class="col-md-1 control-label">Non SKB</label> -->
                      <!-- <div class="col-md-2">
                        <label class="radio radio-inline">
                          <input type="radio" class="radiobox" name="type" value="M" readonly>
                          <span>Masuk</span>
                        </label>
												<label class="radio radio-inline">
  												<input type="radio" class="radiobox" name="type" value="K" readonly>
  												<span>Keluar</span>
											</label>
										</div> -->
                    ...
                    <div class="form-group">
                      <label for="" class="col-md-2 control-label">Non SKB</label>
                      <div class="col-md-2">
                    ...
                    <fieldset>
                      ...
                      <div id="detail">
                        <div class="detailLine form-group">
                          ...
                          <div class="itemLine col-md-1">
                            <input name="qty[]" type="decimal" class="form-control ribuan calc" value="" readonly>
                          </div>
                          <div class="itemLine col-md-1">
                          ...
                          <div class="itemLine col-md-2">
                            <input name="status[]" type="decimal" class="form-control ribuan calc" value="" readonly>
                          </div>
                        </div>
                      </div>
                    </fieldset>
...
$('[name=skb_id]').on('change', function () {
     ...
     $('[name="total[]"]').val('');
     var id = $(this).val();
     url = "{{ route('detail_skb_adjust_finance.get') }}";
     $.ajax({
       url : url,
       type : "POST",
       data : {id:id},
       success : function(data) {
         ...
         if (status == 'IC') {
           ...
           status = 'Cacat Produksi';
         } else if (status == 'PP') {
           status = 'Perbaikan Produksi';
         } else {
           status = 'Kembali Perbaikan';
         }

         $('[name="status[]"]').eq(0).val(status);
         if (data[0]['harga'] !== null) {
           $('[name="harga[]"]').eq(0).val(data[0]['harga']);
           $('[name="total[]"]').eq(0).val(data[0]['total_item']);
         ...
         for (var i = 1; i < data.length; i++) {
           ...
           if (data[i]['harga'] !== null) {
             $('[name="harga[]"]').eq(i).val(data[i]['harga']);
             $('[name="total[]"]').eq(i).val(data[i]['total_item']);
...
$(function () {
  ...
  $('#formAdd').validator().on('submit', function (e) {
    ...
    if (!e.isDefaultPrevented()){
      var id = $('#id').val();
      url = "{{ route('skb_adjust_finance.create') }}";
...
function runselect2one() {
  $('[name="skb_id"]').select2({
    ajax: {
      url: baseurl+'/get_skb_adjust_finance',
      delay:250,
