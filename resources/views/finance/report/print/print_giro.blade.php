  ...
  <table id="table_detail" class="x table table-striped table-bordered table-hover" width="100%">
    <thead>
      <tr style="height:25px;">
        <th class="x" align="center">ID</th>
        <th class="x" align="center">TANGGAL</th>
        <th class="x" align="center">TANGGAL JATUH TEMPO</th>
        <th class="x" align="center">NO GIRO</th>
        <th class="x" align="center">NOMINAL</th>
        <th class="x" align="center">STATUS</th>
      </tr>
    </thead>
    <tbody>
      @if(isset($data))
      ...
      <tr>
        ...
        <td class="x" align="center">{{ Carbon\Carbon::parse($v->tanggal_keluar)->format('d/m/Y')  }}</td>
        <td class="x" align="center">{{ Carbon\Carbon::parse($v->tanggal_jatuh_tempo)->format('d/m/Y')  }}</td>
        <td class="x" align="right"> {{ $v->no_cek_giro_pelanggan }} {{ $v->no_cek_giro_sendiri }}</td>
        <td class="x" align="right"> {{ number_format($v->nominal,2) }}</td>
        <td class="x" align="right"> {{ ($v->status == 'A') ? 'Approved' : 'Process' }}</td>
      </tr>
      @endforeach
